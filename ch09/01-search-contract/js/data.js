...
(function () {
  ...
  function getSuggestedResults(queries, maxSuggestions) {
    ...
    // If we don't have any items, then don't suggest any (just return)
    if (groupedItems || groupedItems.length > 0) {
      ...
      // Test each of the queries and alternatives
      queries.forEach(function (q) {
        ...
        suggestionList.forEach(function (suggestion) {
          ...
          // Test to see if the title matches the query(ies)
          ...
          if (searchExpression.test(suggestion.title)) {
            ...
            // If reached max suggestions, shortcut tests
  ...
  function processPosts(request) {
    ...
    request.items.forEach(function (item) {
      var post = {
        ...
        id: item.id // Used to look up the item
      };
  ...
  WinJS.Namespace.define("Data", {
    ...
    getSuggestedResults: getSuggestedResults, // for Search Suggestions
    resolveGroupByName: resolveGroupByName, // Find group by name (not key)
    appIconStreamReference: appIconStreamReference, // a stream that contains the app's icon
  });
})();
